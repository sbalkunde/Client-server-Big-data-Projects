...
if __name__ == "__main__":
    ...
    fire_df = fire_df.withColumn("Year", year("CallDate"))
    ...
    fire_df_2018 = fire_df.filter(col("Year") == 2018)
    call_types_2018 = fire_df_2018.groupBy("CallType").count().orderBy("count", ascending=False)
    ...
    highest_calls_df = (fire_df_2018.groupBy(month("CallDate").alias("Month")).count().orderBy("count", ascending=False))
    ...
    neighborhood_counts = (fire_df_2018.groupBy("Neighborhood").count().orderBy(col("count").desc()))
    ...
    response_time_neighbourhood_2018 = (fire_df_2018.groupBy("Neighborhood").agg(avg("Delay").alias("AvgResponseTime")).orderBy("AvgResponseTime", ascending=False))
    ...
    fire_calls_week = fire_df_2018.withColumn("WeekOfYear", weekofyear("CallDate")).groupBy("WeekOfYear").count().orderBy("count", ascending=False)
